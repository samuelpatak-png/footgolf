...
/**
 * Soft radial dark blob fading to fully transparent — used as a cheap
 * ground-contact "shadow" decal under the ball and every prop. Not a real
 * shadow-map sample, just enough of a grounding cue that objects stop
 * reading as if they're floating a few pixels above the grass.
 */
export function createShadowBlobTexture(): THREE.Texture {
  const size = 128;
  const canvas = document.createElement("canvas");
  canvas.width = size;
  canvas.height = size;
...
/** Soft white radial glow, additively blended — used as the visible sun disc in the sky. */
export function createSunGlowTexture(): THREE.Texture {
  ...
  const gradient = ctx.createRadialGradient(size / 2, size / 2, 0, size / 2, size / 2, size / 2);
  gradient.addColorStop(0, "rgba(255, 253, 245, 1)");
  gradient.addColorStop(0.25, "rgba(255, 248, 224, 0.85)");
  gradient.addColorStop(0.6, "rgba(255, 240, 200, 0.25)");
